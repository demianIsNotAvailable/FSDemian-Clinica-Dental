...
export const auth = (req: Request, res: Response, next: NextFunction) =>{
    try{
        const decodedPayload = jwt.verify(tokenGenerator(req, res), config.SECRET) as JwtPayload;
...
export const isAdminCheck = (req: Request, res: Response, next: NextFunction) => {
   return req.payload.role == "ADMIN" ? next() : next(new Error('INVALID CREDENTIALS'))
...
export const errorHandler = (err: Error | MongoServerError, req: Request, res: Response, next: NextFunction) =>{
   ...
   console.error(err);
   return res.status(500).json({error: 'SERVER_ERROR patata', err})
}